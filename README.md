...
|---------------|--------------------------------|-----------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------|
| **EP01**      | **Gestión de Usuarios**        | Como usuario, quiero gestionar mi cuenta para acceder a los servicios.      |                                                                                                                                                                                                                                                                                                                                                         |                 |
| HU01          | Registro de usuario web        | Como usuario, quiero registrarme en la plataforma web.                      | **Escenario 1: Registro exitoso**<br>Dado que el usuario está en el formulario de registro,<br>Cuando completa sus datos correctamente,<br>Entonces se crea su cuenta y es redirigido al login.<br><br>**Escenario 2: Campos vacíos**<br>Dado que el usuario no completa los campos requeridos,<br>Cuando intenta registrar,<br>Entonces el sistema muestra mensajes de error. | EP01            |
| HU02          | Inicio de sesión móvil         | Como usuario, quiero iniciar sesión en la app móvil.                        | **Escenario 1: Credenciales válidas**<br>Dado que el usuario ingresa credenciales correctas,<br>Cuando inicia sesión,<br>Entonces accede al panel principal.<br><br>**Escenario 2: Contraseña incorrecta**<br>Dado que el usuario ingresa una contraseña errónea,<br>Cuando intenta iniciar sesión,<br>Entonces el sistema muestra un mensaje de error. | EP01            |
| HT01          | Registro de usuarios (API)     | Como desarrollador, quiero registrar usuarios via API.                       | **Escenario 1: Solicitud válida**<br>Dado que el endpoint `/sign-up` está disponible,<br>Cuando se envía username/password válidos,<br>Entonces se recibe respuesta 201 con datos del usuario.<br><br>**Escenario 2: Rol inválido**<br>Dado que se envía un rol no permitido,<br>Cuando se intenta registrar,<br>Entonces se recibe respuesta 400.       | EP01            |
| **EP02**      | **Pagos y Suscripciones**      | Como usuario, quiero gestionar suscripciones para monitorear sensores.      |                                                                                                                                                                                                                                                                                                                                                         |                 |
| US1           | Crear suscripción al agregar sensor | Como proveedor, quiero vincular suscripciones a sensores nuevos.        | **Escenario 1: Formulario de suscripción**<br>Dado que el proveedor selecciona "Agregar sensor",<br>Cuando completa los datos,<br>Entonces es redirigido al pago.<br><br>**Escenario 2: Pago exitoso**<br>Dado que el pago se confirma,<br>Cuando se procesa,<br>Entonces la suscripción se activa.                                                   | EP02            |
| TS1           | Crear suscripción (API)        | Como desarrollador, quiero crear suscripciones via API.                     | **Escenario 1: Datos válidos**<br>Dado el endpoint `/subscriptions`,<br>Cuando se envía sensorId/residentId válidos,<br>Entonces se recibe 201 con datos de la suscripción.<br><br>**Escenario 2: Sensor ya suscrito**<br>Dado que el sensor tiene suscripción activa,<br>Cuando se intenta crear otra,<br>Entonces se recibe 409.                     | EP02            |
| **EP03**      | **Monitoreo de Agua**          | Como usuario, quiero monitorear el estado de los tanques de agua.           |                                                                                                                                                                                                                                                                                                                                                         |                 |
| US43          | Visualizar estado del tanque   | Como habitante, quiero ver nivel y calidad del agua.                        | **Escenario 1: Datos en tiempo real**<br>Dado que el usuario abre la app,<br>Cuando revisa su tanque,<br>Entonces ve el nivel (%) y calidad (buena/media/mala).<br><br>**Escenario 2: Sensor desconectado**<br>Dado que el sensor no envía datos,<br>Cuando el usuario consulta,<br>Entonces ve un mensaje de "sin conexión".                          | EP03            |
| US45          | Monitoreo de tanques (proveedor) | Como proveedor, quiero revisar tanques asignados.                         | **Escenario 1: Filtro por zona crítica**<br>Dado que hay múltiples tanques,<br>Cuando el proveedor filtra por "crítico",<br>Entonces ve solo los tanques con nivel <10%.                                                                                                                                                                                | EP03            |
| **EP04**      | **Landing Page**               | Como visitante, quiero explorar la solución para evaluar su utilidad.       |                                                                                                                                                                                                                                                                                                                                                         |                 |
| HU13          | Presentación de beneficios     | Como visitante, quiero entender los beneficios.                             | **Escenario 1: Sección de beneficios**<br>Dado que el visitante accede a la página,<br>Cuando navega a "Beneficios",<br>Entonces ve una lista con ventajas clave.                                                                                                                                                                                       | EP04            |
| HU16          | Comparación de planes         | Como visitante, quiero comparar servicios.                                  | **Escenario 1: Visualización de precios**<br>Dado que el visitante ve "Nuestros Servicios",<br>Cuando revisa los planes,<br>Entonces puede solicitar asesoría.                                                                                                                                                                                          | EP04            |
| **EP05**      | **Analytics**                  | Como usuario, quiero visualizar métricas de sensores.                       |                                                                                                                                                                                                                                                                                                                                                         |                 |
| HU19          | Métricas móviles              | Como habitante, quiero ver datos básicos en mi móvil.                       | **Escenario 1: Resumen rápido**<br>Dado que el usuario abre la app,<br>Cuando ve el dashboard,<br>Entonces visualiza consumo actual y nivel del tanque.                                                                                                                                                                                                 | EP05            |
| HU18          | Dashboard para proveedores    | Como proveedor, quiero analizar métricas.                                  | **Escenario 1: Exportar reporte**<br>Dado que el proveedor selecciona un período,<br>Cuando hace clic en "Exportar",<br>Entonces recibe un PDF con los datos.                                                                                                                                                                                           | EP05            |
...
La siguiente matriz (LACX) indica los líderes (L) y colaboradores (C) por cada aspecto del sistema abordado en este segundo:
